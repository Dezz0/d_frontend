...
export const useCreateApplication = () => {
    ...
    mutation: {
      ...
      onError: (error: any) => {
        ...
        if (error?.status === 400) {
          ...
          } else if (error?.data?.detail?.includes('Room not in selected rooms')) {
            errorMessage = 'Датчики выбраны для несуществующей комнаты';
          } else if (error?.data?.detail?.includes('Admin cannot create applications')) {
  ...
  const handleAddRoom = () => {
    setRooms([...rooms, { id: Date.now(), selectedRoomId: null, sensorIds: [] }]);
  };
  ...
  // Изменить датчик
  const handleSensorChange = (roomIndex: number, sensorIndex: number, sensorId: number) => {
    const newRooms = [...rooms];
  ...
  const handleSubmit = () => {
    ...
    rooms.forEach((room, index) => {
      ...
      room.sensorIds.forEach((sensorId, sensorIndex) => {
        if (sensorId === -1) {
          errors.push(`Комната #${index + 1}: не выбран датчик #${sensorIndex + 1}`);
        }
      });
    });
    ...
    const selectedRooms = rooms.map(room => room.selectedRoomId!).filter(Boolean);
  ...
  const isLoading = isLoadingDictionaries || createApplicationMutation.isPending || isFetchingUser;
