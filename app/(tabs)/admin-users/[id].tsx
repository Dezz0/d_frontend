...
export default function UserApplicationsScreen() {
  ...
  const {
    ...
    isLoading,
    error
  } = useGetUserApplicationsApplicationsAdminUserIdApplicationsGet(parseInt(userId!));
    ...
    return (
      <View style={[styles.container, { paddingTop: insets.top }]}>
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Загрузка заявок пользователя...</Text>
        </View>
      </View>
    );
  }
  ...
  return (
    <View style={[
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <TouchableOpacity style={styles.backButton} onPress={handleBack}>
          <Text style={styles.backButtonText}>← Назад к списку пользователей</Text>
        </TouchableOpacity>

        {applications && applications.length > 0 ? (
          <>
            <View style={styles.summarySection}>
              <Text style={styles.summaryTitle}>Всего заявок: {applications.length}</Text>
              <Text style={styles.summaryText}>
                • В ожидании: {applications.filter(a => a.status === 'pending').length}
              </Text>
              <Text style={styles.summaryText}>
              ...
              >
                <View style={styles.applicationHeader}>
                  ...
                  <View style={[
                    ...
                    {
                      backgroundColor:
                        application.status === 'pending' ? '#FA8C16' :
                          application.status === 'approved' ? '#52C41A' : '#FF4D4F'
                    }
                  ]}>
                    <Text style={styles.statusText}>
                      {application.status === 'pending' ? 'В ожидании' :
                ...
                {application.rejection_comment && application.status === 'rejected' && (
                  <View style={styles.commentBox}>
                    <Text style={styles.commentLabel}>Комментарий:</Text>
                    <Text style={styles.commentText}>{application.rejection_comment}</Text>
                  </View>
                )}
              </TouchableOpacity>
            ))}
